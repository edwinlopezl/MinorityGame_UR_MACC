...
class Mercado:
    ...
    def actualizarPrecio(self, arreglo_decisiones):
        ...
        numerador = np.clip(len(arreglo_decisiones) + A_t, 1e-3, len(arreglo_decisiones) - 1e-3)
        denominador = np.clip(len(arreglo_decisiones) - A_t, 1e-3, len(arreglo_decisiones) - 1e-3)

        self.precio *= numerador / denominador
